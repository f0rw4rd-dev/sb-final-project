# ADR-02: Использование контейнеризации (Docker/Kubernetes)

1. Контекст:
   * Нужно выбрать способ упаковки и развертывания сервисов.
2. Решение:
   * Каждый сервис упаковывается в Docker-образ, оркестрация происходит через Kubernetes.
3. Статус:
   * Принято.
4. Обоснование:
   * Универсальный подход для развертывания в multi-cloud среде (AWS, Azure).
   * Упрощает масштабирование и обновление сервисов без downtime.
   * Позволяет внедрять Service Mesh.
5. Последствия:
   * Повышается сложность инфраструктуры (необходимо развернуть Kubernetes-кластер, настраивать Service Mesh).
   * Требуются компетенции DevOps/SRE в команде.
