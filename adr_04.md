# ADR-04: Выделение БД для аналитики

1. Контекст:
   * Аналитические задачи (агрегированные метрики, рекомендации, сравнительный анализ тренировок).
2. Решение:
   * Для аналитики создать отдельное хранилище.
3. Статус:
   * Принято.
4. Обоснование:
   * Разделение OLTP и OLAP позволяет не нагружать основную БД сложными запросами и обеспечивает масштабируемость аналитических вычислений.
   * Использование облачных сервисов Big Data сокращает время и затраты на собственную инфраструктуру.
5. Последствия:
   * Потребуется организация ETL-процессов (Extract-Transform-Load).
   * Стоимость Big Data сервисов может расти вместе с объемами данных.
